...
def _condition_context(
    ...
    for metric_name, spec in metric_specs.items():
        ...
        lines.append(f"- {spec.label}: {value:.12g}")

    if condition == "metrics":
        return "\n".join(lines) + "\n"
    if condition == "interpreted":
        return "\n".join(lines) + "\n" + interpreted_feedback.strip() + "\n"
...
def _mutation_record_id(experiment: ExperimentConfig) -> str | None:
    if experiment.mutation is None:
        return None
...
def _audit_records(
    ...
    expected_metadata = {
        ...
        "model": model_id,
        "mutation": _mutation_record_id(experiment),
...
def _validate_experiment(experiment: ExperimentConfig) -> None:
    if experiment.runner not in {"repair", "feedback_replication"}:
        raise ValueError("Repair experiments require runner: repair")
    if experiment.mutation is None:
        raise ValueError("Repair experiments require a mutation")
    if experiment.runner == "repair" and experiment.task is None:
        raise ValueError("Config-driven repair experiments require a task")
    if not experiment.conditions:
        raise ValueError("Repair experiments require at least one condition")
